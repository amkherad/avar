...
static bool queue_has_running_work(const char *queue_id) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        const bool keeps_running = queue_item_keeps_running(status);
        free(status);
        if (keeps_running) {
            return true;
        }
...
static QueueError queue_set_started(const char *id, const bool started) {
    ...
    cJSON_ReplaceItemInObjectCaseSensitive(obj, AVAR_QUEUE_FIELD_STARTED,
                                           cJSON_CreateBool(started));
    char *updated = cJSON_PrintUnformatted(obj);
    cJSON_Delete(obj);
    if (updated == NULL) {
        return QueueErrorPersist;
...
void queue_sync_started_state(const char *queue_id) {
    ...
    if (queue_has_running_work(queue_id)) {
        return;
    }

    char *name = NULL;
    const int queue_index = queue_find_index(AVAR_FIELD_ID, queue_id);
    if (queue_index >= 0) {
        name = get_config_array_item_field(AVAR_CFG_DM_QUEUES, (size_t)queue_index,
                                           AVAR_QUEUE_FIELD_NAME);
    }

    if (queue_set_started(queue_id, false) == QueueErrorNone) {
        LOG_INFO("Queue '%s' (%s) stopped automatically (no active or queued downloads)",
                 name != NULL ? name : queue_id, queue_id);
    }
