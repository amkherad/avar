...
Toggle the detail panel and console from footer buttons. When **Histogram + values** is enabled for monitors, memory/CPU/network labels stay on one line with values overlaid on the chart.

## Responsive behavior

On narrow viewports the sidebar and detail panel collapse to keep the download list usable. Resize handles are hidden on touch devices.
